...
static int mkdirs(const char* path) {
  ...
  wchar_t* wpath = (wchar_t*)malloc(l * sizeof(wchar_t));
  if (wpath == NULL) return 1;
  MultiByteToWideChar(CP_UTF8, 0, path, -1, wpath, l);
...
acb* acb_open_awb(const char* path) {
#ifdef _WIN32
  int len = MultiByteToWideChar(CP_UTF8, 0, path, -1, NULL, 0);
  wchar_t* wpath = (wchar_t*)malloc(len * sizeof(wchar_t));
  if (!wpath) return NULL;
...
int acb_get_row_data(acb* acbp, uint32_t row_index, const char* column_name, void* res, size_t buf_size) {
  if (!acbp) return 0;
  ...
  if (track_list) {
    ...
    for (uint16_t i = 0; i < cue_table->utf_header.row_length; i++) {
      ...
      if (b < wave_form_table->utf_header.row_length) {
        ...
        if (res == 0) {
          if (streaming) {
            acb_get_row_data(wave_form_table, b, "StreamAwbId", &wav_id, 0);
          } else {
...
int acb_extract(acb* acbp, const char* target_dir, acb_extract_callback callback) {
  ...
  acb* awb = NULL;
  if (desc_awb.length == 0) {
    char awbpath[MAX_PATH_LENGTH] = { 0 };
    char awbpath2[MAX_PATH_LENGTH] = { 0 };
    strcpy(awbpath, acbp->path);
    char* dot = strrchr(awbpath, '.');
    if (dot) {
      *dot = '\0';
      strcpy(awbpath2, awbpath);
      strcat(awbpath2, ".awb");
      awb = acb_open_awb(awbpath2);
    } else {
      strcat(awbpath, ".awb");
      awb = acb_open_awb(awbpath);
    }
  } else {
    awb = acb_memopen_awb(acbp, 8 + acbp->utf_header.binary_data_offset + desc_awb.offset, desc_awb.length);
  }
  ...
  int mkdirres = mkdirs(target_dir);
  if (mkdirres) {
    free(files);
    free(file_end_points);
    free(id_arr);
